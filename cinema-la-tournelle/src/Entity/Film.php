...
define('AFFICHESDIRECTORY', 'https://image.tmdb.org/t/p/w500');
...
class Film
{
    ...
    {
        if (!$this->url) {
            $filmTitre = $this->getTitre();
            $APIMOVIE = '20c7b8d480d4a435703ad88460e996a5';

            $apiUrl = "https://api.themoviedb.org/3/search/movie?api_key={$APIMOVIE}&query={$filmTitre}";
            ...
            if ($data->results && count($data->results) > 0) {
                ...
                if ($posterPath) {
                    $this->url = AFFICHESDIRECTORY . $posterPath;
                }
            }
